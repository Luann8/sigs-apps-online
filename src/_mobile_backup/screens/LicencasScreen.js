...
import { getTipoLicencaConfig, getStatusConfig, getDaysUntilExpiry, formatDate } from '../utils/formatters';
...
export function LicencasScreen({ navigation }) {
  ...
  const allLicencas = useMemo(
    () => rawAllLicencas.map((l) => ({ ...l, id: l._id })),
    [rawAllLicencas]
  );
  const filtered = getFilteredLicencas(allLicencas);
  const [localSearch, setLocalSearch] = useState(searchQuery);
  const insets = useSafeAreaInsets();

  const { countUnseen } = useAlertsStore();
  const alertas = buildAlertList(allLicencas);
        ...
        <View style={styles.headerTop}>
          ...
          <View style={{ flex: 1 }}>
            ...
            >
              ...
              <Text style={styles.headerEstabText} numberOfLines={1}>
                {estabelecimentoAtual ? estabelecimentoAtual.nome : 'Selecionar Estabelecimento'}
              </Text>
              <MaterialCommunityIcons name="swap-horizontal" size={14} color="rgba(255,255,255,0.8)" />
            </TouchableOpacity>
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={styles.emptySubtitle}>
              {!allLicencas || allLicencas.length === 0
...
const styles = StyleSheet.create({
  ...
  headerSubtitle: { ...Typography.caption, color: 'rgba(255,255,255,0.7)', marginTop: 4 },
